...
10. **Баг апстрима:** `deploy/install.sh` ставит `umask 077` и под ним же запускает
    `go test ./...`. Тест `TestLoadAcceptsSystemdCredentialReadPermissions` создаёт файл
    с режимом `0444`, который под этим umask превращается в `0400`, негативная ветка теста
    не срабатывает, и установка обрывается на полностью исправном сервере. Обходной путь —
    запуск тестов под `umask 022` — применяется к клону автоматически.
